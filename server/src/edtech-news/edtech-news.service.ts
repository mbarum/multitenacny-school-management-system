...
import { EdTechArticle, ArticleStatus } from '../entities/edtech-article.entity';
import { randomUUID } from 'crypto';

@Injectable()
export class EdTechNewsService implements OnModuleInit {
  private readonly logger = new Logger(EdTechNewsService.name);

  constructor(
    @InjectRepository(EdTechArticle)
    private readonly articleRepo: Repository<EdTechArticle>,
  ) {}
  ...
  async seedInitialArticlesIfEmpty() {
    try {
      ...
      if (count === 0) {
        this.logger.log('Seeding initial EdTech articles...');
        const initialArticles: Partial<EdTechArticle>[] = [
          {
        ...
        for (const art of initialArticles) {
          const entity = this.articleRepo.create(art);
          await this.articleRepo.save(entity);
        }
        this.logger.log(`Successfully seeded ${initialArticles.length} EdTech articles`);
      }
    } catch (e: any) {
      this.logger.error(`Error during EdTech articles seed: ${e.message}`);
    }
  }

  async findAll(params?: { category?: string; status?: string }): Promise<EdTechArticle[]> {
    ...
    if (params?.category && params.category !== 'ALL') {
      qb.andWhere('article.category = :category', { category: params.category });
    }

    qb.orderBy('article.featured', 'DESC');
    qb.addOrderBy('article.createdAt', 'DESC');

    return qb.getMany();
  }

  async findOne(idOrSlug: string): Promise<EdTechArticle> {
    ...
    if (!article) {
      throw new NotFoundException(`Article with ID or slug "${idOrSlug}" not found`);
    }

    // Increment views count asynchronously
    this.articleRepo.increment({ id: article.id }, 'viewsCount', 1).catch(() => {});
    article.viewsCount = (article.viewsCount || 0) + 1;

    return article;
  }

  async create(data: Partial<EdTechArticle>): Promise<EdTechArticle> {
    const id = data.id || data.slug || `art-${randomUUID().slice(0, 8)}`;
  ...
  async update(id: string, data: Partial<EdTechArticle>): Promise<EdTechArticle> {
  ...
  async delete(id: string): Promise<{ success: boolean; message: string }> {
    ...
    return { success: true, message: `Article "${article.title}" removed successfully` };
  }

  async toggleStatus(id: string): Promise<EdTechArticle> {
    const article = await this.articleRepo.findOne({ where: { id } });
    if (!article) {
      throw new NotFoundException(`Article with ID "${id}" not found`);
    }

    article.status = article.status === ArticleStatus.PUBLISHED ? ArticleStatus.DRAFT : ArticleStatus.PUBLISHED;
    return this.articleRepo.save(article);
  }

  async toggleFeatured(id: string): Promise<EdTechArticle> {
    const article = await this.articleRepo.findOne({ where: { id } });
    if (!article) {
      throw new NotFoundException(`Article with ID "${id}" not found`);
    }
