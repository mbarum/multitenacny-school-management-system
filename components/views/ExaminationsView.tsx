...
const ExaminationsView: React.FC = () => {
    ...
    return (
        <div className="p-6 md:p-8">
            ...
            {activeTab === 'manage' && <ManageExamsView exams={exams} classes={classes} openModal={(exam) => { setEditingExam(exam); setIsModalOpen(true); }} />}
        ...
        {selectedSubjectId && <>
            <table className="w-full text-left"><thead><tr className="bg-slate-50 border-b"><th>Student</th><th>{selectedExamType === ExamType.Traditional ? 'Score' : 'Level'}</th><th>Comments</th></tr></thead>
                <tbody>{studentsInClass.map((student:Student) => (<tr key={student.id} className="border-b"><td className="p-2">{student.name}</td>
                    <td className="p-2">{selectedExamType === ExamType.Traditional ? <input type="number" value={gradeEntries.get(student.id)?.score || ''} onChange={e => setGradeEntries(new Map(gradeEntries.set(student.id, {...gradeEntries.get(student.id), score: parseInt(e.target.value)})))} className="w-20 p-1 border rounded"/> : <select value={gradeEntries.get(student.id)?.cbetScore || ''} onChange={e => setGradeEntries(new Map(gradeEntries.set(student.id, {...gradeEntries.get(student.id), cbetScore: e.target.value as CbetScore})))} className="p-1 border rounded">{Object.values(CbetScore).map(v => <option key={v} value={v}>{v}</option>)}</select>}</td>
                    <td className="p-2"><input type="text" value={gradeEntries.get(student.id)?.comments || ''} onChange={e => setGradeEntries(new Map(gradeEntries.set(student.id, {...gradeEntries.get(student.id), comments: e.target.value})))} className="w-full p-1 border rounded"/></td></tr>))}</tbody>
            </table>
