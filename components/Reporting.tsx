...
const AttendanceReport: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const filteredRecords = useMemo(() => {
        return attendanceRecords.filter(r => {
            if (r.classId !== classId || r.status === AttendanceStatus.Present) return false;
            const recordDate = new Date(r.date);
            if (startDate && recordDate < new Date(startDate)) return false;
            if (endDate && recordDate > new Date(endDate)) return false;
            return true;
        }).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }, [attendanceRecords, classId, startDate, endDate]);
...
const CashFlowProjection: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    const { students, feeStructure, expenses, classes } = useData();
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg">
                ...
                <div className="w-full h-96 mb-8">
                 ...
                 <table className="w-full text-left table-auto">
                    ...
                    <tbody>
                        {/* Fix: Explicitly typed 'd' to resolve property access error on 'unknown'. */}
