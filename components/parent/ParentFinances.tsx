...
const ParentFinances: React.FC = () => {
    const { parentChildren, studentFinancials, transactions, addTransaction, darajaSettings, addNotification } = useData();
    ...
    const openPaymentModal = (child: Student) => {
        ...
        const balance = studentFinancials[child.id]?.balance || 0;
    ...
    const handlePayFees = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // The transaction is recorded by the backend via the M-Pesa callback.
            // The frontend will get the update via websocket or a future data refresh.
    ...
    return (
        <div className="p-6 md:p-8">
            ...
            {parentChildren.map(child => {
                 ...
                 const childTransactions = transactions.filter(t => t.studentId === child.id && t.type === 'Payment').slice(0, 3);
                 return (
                    <div key={child.id} className="bg-white p-6 rounded-xl shadow-lg mb-6">
                        ...
                        <div className="mt-4 border-t pt-4">
                            ...
                            <table className="w-full text-left text-sm">
                                <tbody>
                                    {childTransactions.map(p => (
                                        <tr key={p.id}><td>{p.date}</td><td>{p.method}</td><td>{p.transactionCode}</td><td className="text-right font-medium">KES {p.amount.toLocaleString()}</td></tr>
                                    ))}
