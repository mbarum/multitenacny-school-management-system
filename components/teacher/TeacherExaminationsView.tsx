...
const TeacherExaminationsView: React.FC = () => {
    const { exams, grades, updateGrades, assignedClass, students, classSubjectAssignments, subjects, currentUser } = useData();
    ...
    }, [selectedExamId, selectedSubjectId, studentsInClass, grades]);
    
    const handleGradeChange = (studentId: string, value: Partial<{ score: number | null, cbetScore: CbetScore | null, comments: string }>) => {
        setGradeEntries(prev => new Map(prev).set(studentId, { ...prev.get(studentId)!, ...value }));
    };

    const handleSaveGrades = () => {
        if (!selectedExamId || !selectedSubjectId) return;
        const otherGrades = grades.filter(g => g.examId !== selectedExamId || g.subjectId !== selectedSubjectId);
