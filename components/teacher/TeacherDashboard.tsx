...
const TeacherDashboard: React.FC = () => {
    const { students, assignedClass, attendanceRecords } = useData();
    ...
    const todaysAttendance = attendanceRecords.filter(r => r.date === today && r.classId === assignedClass.id);
    const presentToday = todaysAttendance.filter(r => r.status === AttendanceStatus.Present || r.status === AttendanceStatus.Late).length;
    const attendancePercentage = studentsInClass.length > 0 ? Math.round((presentToday / studentsInClass.length) * 100) : 0;
