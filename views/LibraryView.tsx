...
const LibraryView: React.FC = () => {
    ...
    const { data: studentsList = [] } = useQuery({
        ...
        queryFn: () => api.getStudents({ mode: 'minimal', limit: 1000 }).then((res: any) => Array.isArray(res) ? res : res.data),
    ...
    const { data: activeLoans = [], refetch: refetchLoans } = useQuery({
        ...
        queryFn: () => api.getLibraryTransactions({ limit: 100, status: LibraryStatus.BORROWED }).then((res: any) => Array.isArray(res) ? res : res.data),
        enabled: activeTab === 'circulation'
    });
