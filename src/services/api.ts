...
export const getStudents = (params: any = {}): Promise<any> => {
    ...
    // Only append params that actually have values to avoid sending "undefined" as a string
...
export const getTransactions = (params: any = {}): Promise<any> => {
    ...
    Object.entries(params).forEach(([k, v]) => {
        if (v !== undefined && v !== null && v !== '') query.append(k, (v as any).toString());
    });
...
export const exportStudents = (): Promise<Blob> => apiFetch('/students/export', { responseType: 'blob' } as any);
...
export const exportExpenses = (params: any): Promise<Blob> => {
    const query = new URLSearchParams(params);
    return apiFetch(`/expenses/export?${query.toString()}`, { responseType: 'blob' } as any);
}
