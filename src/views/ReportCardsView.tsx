...
const ReportCardModal: React.FC<ReportCardModalProps> = ({ isOpen, onClose, student, exam, grades, subjects, gradingScale, schoolInfo, loading }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Institutional Performance Data" size="3xl" footer={<button onClick={() => window.print()} className="px-10 py-4 bg-slate-900 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl no-print hover:scale-105 transition-all cursor-pointer">Export Official PDF</button>}>
            ...
            <div className="printable-area p-6 bg-white text-slate-900 font-sans border-[6px] border-slate-50 m-2 min-h-[297mm]">
                ...
                {/* Framework Selector Bar (Hidden on print) */}
                <div className="no-print mb-6 p-3 bg-slate-100 rounded-2xl flex flex-wrap items-center justify-between gap-3 border border-slate-200">
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Evaluation View:</span>
                        <span className="text-xs font-bold text-slate-700">
                            {viewMode === GradingSystem.CBC ? 'Competency-Based Assessment (CBC)' : 'Traditional Percentage Scale (Marks)'}
                        </span>
                    </div>
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                        <thead>
                            <tr className="bg-slate-900 text-white">
                                ...
                                <th className="p-3 font-black uppercase tracking-widest text-[9px] border-r border-slate-700 text-center w-24">
                                    {viewMode === GradingSystem.CBC ? 'Level' : 'Marks'}
                                </th>
                                <th className="p-3 font-black uppercase tracking-widest text-[9px] border-r border-slate-700 text-center w-24">
                ...
                {viewMode === GradingSystem.Traditional ? (
                    ...
                    <div className="flex gap-4 mb-8 justify-center">
                        <div className="bg-slate-900 text-white px-6 py-3 rounded-xl text-center shadow-lg">
                            <p className="text-[8px] font-black uppercase tracking-widest mb-0.5 opacity-50">Total Points</p>
                            <p className="text-xl font-black">{totalCbcPoints}</p>
                        </div>
                        <div className="bg-primary-600 text-white px-6 py-3 rounded-xl text-center shadow-lg">
                            <p className="text-[8px] font-black uppercase tracking-widest mb-0.5 opacity-70">Mean Points</p>
                            <p className="text-xl font-black">{avgCbcPoints.toFixed(1)} / 4.0</p>
                        </div>
                        <div className="bg-emerald-600 text-white px-6 py-3 rounded-xl text-center shadow-lg">
                            <p className="text-[8px] font-black uppercase tracking-widest mb-0.5 opacity-80">Overall Level</p>
                            <p className="text-xs font-black">{meanCbcLevel}</p>
                        </div>
                    </div>
                )}
