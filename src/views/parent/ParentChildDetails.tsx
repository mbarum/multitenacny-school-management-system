...
const ParentChildDetails: React.FC = () => {
    ...
    const { data: transactions = [], isLoading: loadingFinance } = useQuery({
        ...
        queryFn: () => selectedChild ? api.getTransactions({ studentId: selectedChild.id, limit: 50 }).then((res: any) => Array.isArray(res?.data) ? res.data : (Array.isArray(res) ? res : [])) : Promise.resolve([]),
        enabled: !!selectedChild
    });
