...
export class LocalAuthGuard extends AuthGuard('emis-local') {
  handleRequest<TUser = any>(err: any, user: TUser, info: any): TUser {
    if (err || !user) {
      ...
      console.log(
        `[LocalAuthGuard] Registered strategies: ${registeredStrategies.join(', ')}`,
      );

      // Check for specific passport errors
      if (
        infoMessage === 'Unknown authentication strategy "emis-local"' ||
        infoMessage === 'Unknown authentication strategy "local"'
      ) {
      ...
      if (infoMessage) {
        throw new UnauthorizedException(
          `Authentication failed: ${infoMessage}`,
        );
      }
