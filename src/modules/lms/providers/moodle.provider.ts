...
export class MoodleProvider implements LmsProvider {
  ...
  constructor(
    ...
    this.token = this.cryptoService.decrypt(this.connection.encryptedCredential1);
  }
  ...
  async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.log(`Successfully connected to Moodle site: ${response.sitename}`);
    } catch (error) {
  ...
  async syncStudents(): Promise<LmsStudent[]> {
    const users = await this.makeRequest('core_user_get_users', { criteria: [{ key: 'email', value: '%' }] });
  ...
  async getGrades(studentId: string): Promise<LmsGrade[]> {
    ...
    console.warn('getGrades for Moodle is a complex operation and this is a placeholder.');
    return [];
  }
   ...
   */
  private async makeRequest(wsfunction: string, args: Record<string, any> = {}): Promise<any> {
