...
const SuperAdminDashboard: React.FC = () => {
    ...
    const handleSavePricing = async (e: React.FormEvent) => {
        ...
        try {
            ...
            addNotification('Platform pricing updated successfully.', 'success');
            setIsPricingModalOpen(false);
            fetchData();
        } catch (error) {
            addNotification('Failed to update pricing.', 'error');
        }
    };
    
    const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setNewPricing(prev => ({ ...prev, [e.target.name]: parseFloat(e.target.value) || 0 }));
    };
        ...
        <div className="p-6 md:p-8">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-slate-500">Manage all schools and monitor system health.</p>
                </div>
            ...
            <Modal isOpen={isPricingModalOpen} onClose={() => setIsPricingModalOpen(false)} title="Platform Pricing" size="lg">
                <form onSubmit={handleSavePricing} className="space-y-6">
                    <p className="text-sm text-slate-600">Update the pricing for SaaS subscriptions. These prices will be reflected on the landing page and new registrations.</p>
