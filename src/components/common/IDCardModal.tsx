...
const IDCardFront: React.FC<{ person: Student | Staff, isStudent: boolean, schoolInfo: SchoolInfo }> = ({ person, isStudent, schoolInfo }) => {
    ...
    // Generate a shorter ID for staff display if needed
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex justify-center items-center p-4 backdrop-blur-sm">
            ...
            <div className="relative">
                <button 
                    onClick={onClose} 
                    className="absolute -top-12 right-0 text-white hover:text-slate-200 transition-colors no-print flex items-center"
                >
                ...
                <div className="perspective printable-card-area">
                     <div 
                        className={`relative w-[540px] h-[340px] transition-transform duration-700 preserve-3d card-container ${isFlipped ? 'rotate-y-180' : ''}`}
                     >
                ...
                <div className="mt-8 flex justify-center space-x-6 no-print">
                    <button 
                        ...
                        className="px-6 py-2.5 bg-white text-slate-800 font-bold rounded-full shadow-lg hover:bg-slate-100 hover:scale-105 transition-all flex items-center"
                    >
