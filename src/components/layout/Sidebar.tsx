    ...
    <div className="flex items-center justify-center py-5 px-4 border-b border-slate-200 dark:border-slate-800 h-16 bg-white dark:bg-slate-900 transition-colors">
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : ''} w-full`}>
            {logoUrl ? (
                ...
                <div className="h-10 w-10 flex-shrink-0 flex items-center justify-center rounded-full text-primary-600 dark:text-primary-400 bg-primary-100 dark:bg-primary-950/60">
            ...
            <div className={`ml-3 transition-opacity duration-300 ease-in-out ${isCollapsed ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>
                <h1 className="text-lg font-bold truncate max-w-[150px] text-primary-800 dark:text-primary-300">
                    {schoolName}
                </h1>
            </div>
        </div>
...
const Sidebar: React.FC = () => {
    ...
    return (
        <>
            ...
            {/* Sidebar */}
            <aside 
                className={`fixed top-0 left-0 h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col z-40 transition-all duration-300 ease-in-out shadow-xl
            ...
            >
                ...
                <div className={`px-4 py-4 mt-auto border-t border-slate-200 dark:border-slate-800 transition-opacity duration-300 ${isSidebarCollapsed ? 'opacity-0' : 'opacity-100'}`}>
