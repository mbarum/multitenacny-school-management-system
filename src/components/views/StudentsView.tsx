...
const StudentsView: React.FC = () => {
    ...
    const students = Array.isArray(registry?.data) ? registry.data : (Array.isArray(registry) ? registry : []);
            ...
            <Modal isOpen={isAddModalOpen} onClose={()=>setIsAddModalOpen(false)} title="Mass Enrollment Gateway" size="xl">
                <form onSubmit={e=>{e.preventDefault(); enrollMutation.mutate(newStudent);}} className="space-y-8">
                    ...
                    <div className="p-4 bg-primary-50 rounded-2xl border border-primary-100 flex items-start gap-3">
                        <input 
                            id="comp-enroll-notify-email"
                            type="checkbox" 
                            checked={newStudent.notifyGuardianEmail ?? true} 
                            onChange={e => setNewStudent({ ...newStudent, notifyGuardianEmail: e.target.checked })} 
                            className="mt-1 h-4 w-4 rounded text-primary-600 focus:ring-primary-500 cursor-pointer"
                        />
                        <div className="flex-1">
                            <label htmlFor="comp-enroll-notify-email" className="block text-xs font-bold text-slate-900 cursor-pointer">
                                Auto-Provision Parent Portal Account & Send Credentials
                            </label>
                            <p className="text-[11px] text-slate-600 mt-0.5">
                                Automatically creates an active portal account for {newStudent.guardianEmail || 'guardian email'} and dispatches login URL, username, and temporary password.
                            </p>
                        </div>
                    </div>
