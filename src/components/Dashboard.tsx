...
const StatCard: React.FC<{ title: string; value: string; icon: React.ReactElement, loading?: boolean, onClick?: () => void }> = ({ title, value, icon, loading, onClick }) => (
    <div 
        onClick={onClick}
        className={`bg-white p-6 rounded-xl shadow-lg flex items-center space-x-4 transition-transform transform hover:-translate-y-1 h-32 ${onClick ? 'cursor-pointer hover:shadow-xl' : ''}`}
    >
